...
func TestNewClientBuildsEnvelopeURLFromDSN(t *testing.T) {
	...
	if c.authHeader != "Sentry sentry_version=7, sentry_client=rainbond-operator-go/1.0, sentry_key=public" {
	...
	if c.component != "rainbond-operator" {
...
func TestNewClientRejectsInvalidDSN(t *testing.T) {
	if _, err := newClient(Config{Enabled: true, DSN: "https://example.invalid/42"}, "rainbond-operator"); err == nil {
