...
func (r *RbdcomponentMgr) UpdateOrCreateResource(obj client.Object) (reconcile.Result, error) {
	...
	if !resourceNeedsUpdate(oldOjb, obj) {
		r.log.V(5).Info("Resource already matches desired state, skipping update",
...
// resourceNeedsUpdate compares the fields managed by the operator. In
// particular, monitor is reconciled every five seconds while it is not ready;
// updating unchanged ConfigMaps or Services during that requeue creates
// unnecessary API events and can cause dependent workloads to be restarted.
func resourceNeedsUpdate(old, desired client.Object) bool {
	switch desired := desired.(type) {
	case *appsv1.StatefulSet:
		return statefulSetNeedsUpdate(old.(*appsv1.StatefulSet), desired)
	case *corev1.ConfigMap:
		current := old.(*corev1.ConfigMap)
		return metadataNeedsUpdate(current.ObjectMeta, desired.ObjectMeta) ||
			!apiequality.Semantic.DeepDerivative(desired.Data, current.Data) ||
			!apiequality.Semantic.DeepDerivative(desired.BinaryData, current.BinaryData) ||
			!apiequality.Semantic.DeepDerivative(desired.Immutable, current.Immutable)
	case *corev1.Service:
		current := old.(*corev1.Service)
		return metadataNeedsUpdate(current.ObjectMeta, desired.ObjectMeta) ||
...
func metadataNeedsUpdate(old, desired metav1.ObjectMeta) bool {
	return !apiequality.Semantic.DeepDerivative(desired.Labels, old.Labels) ||
		!apiequality.Semantic.DeepDerivative(desired.Annotations, old.Annotations) ||
		!apiequality.Semantic.DeepDerivative(desired.OwnerReferences, old.OwnerReferences)
}
...
func (r *RbdcomponentMgr) updateRuntimeObject(old, new client.Object) client.Object {
	...
	if n, ok := new.(*corev1.ConfigMap); ok {
		o := old.(*corev1.ConfigMap)
		n.ResourceVersion = o.ResourceVersion
		if n.Annotations[handler.MonitorConfigMapOwnerAnnotation] != handler.MonitorName {
			n.Data = preserveAdditionalConfigMapData(o.Data, n.Data)
			n.BinaryData = preserveAdditionalConfigMapData(o.BinaryData, n.BinaryData)
		}
		return n
	}
...
// preserveAdditionalConfigMapData keeps keys that are not generated by the
// operator for ConfigMaps that support externally managed data. The monitor
// ConfigMap is intentionally excluded: its complete content is operator-owned.
func preserveAdditionalConfigMapData[T ~map[string]V, V any](current, desired T) T {
	if len(current) == 0 {
		return desired
	}
	if desired == nil {
		desired = make(T, len(current))
	}
