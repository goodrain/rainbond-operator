...
func (p *pkg) imagesLoadAndPush() error {
	...
	//
	//	f := func() (bool, error) {
	//		oldimage, err := p.imageLoad(pstr)
	//		if err != nil {
	//			l.Error(err, "load image")
	//			return false, fmt.Errorf("load image: %v", err)
	//		}
	//
	//		newImage := newImageWithNewDomain(oldimage, rbdutil.GetImageRepository(p.cluster))
	//		if newImage == "" {
	//			return false, fmt.Errorf("parse image name failure")
	//		}
	//
	//		image, err := p.ctr.ImageService.Get(p.ctr.CCtx, oldimage)
	//		if err != nil {
	//			return false, fmt.Errorf("get image %s failure: %v", oldimage, err)
	//		}
	//		image.Name = newImage
	//		if _, err = p.ctr.ImageService.Create(p.ctr.CCtx, image); err != nil {
	//			// If user has specified force and the image already exists then
	//			// delete the original image and attempt to create the new one
	//			if errdefs.IsAlreadyExists(err) {
	//				if err = p.ctr.ImageService.Delete(p.ctr.CCtx, newImage); err != nil {
	//					return false, fmt.Errorf("delete image %s failure: %v", newImage, err)
	//				}
	//				if _, err = p.ctr.ImageService.Create(p.ctr.CCtx, image); err != nil {
	//					return false, fmt.Errorf("create image %s failure: %v", newImage, err)
	//				}
	//			} else {
	//				return false, fmt.Errorf("create image %s failure: %v", newImage, err)
	//			}
...
func (p *pkg) checkIfImageExists(image string) (bool, error) {
	...
	//defer cancel()
	//p.ctr.ImageService.List(p.ctr.CCtx, "")
